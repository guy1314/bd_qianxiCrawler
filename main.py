...
def crawl(scale):
    if scale == '210100':
        try:
            ...
            city_date_curve_index = calculate_index(
                city_history_curve, city_date_curve_rank, date)
            ...
            city_date_curve_rank.to_csv(
                rf'{FILE_ROOT_PATH}/城市当日迁徙占比/{direction}/{city}_{date}.csv', encoding='utf-8_sig')
            city_history_curve.to_csv(
                rf'{FILE_ROOT_PATH}/城市历史迁徙指数/{direction}/{city}.csv', encoding='utf-8_sig')
            city_date_curve_index.to_csv(
                rf'{FILE_ROOT_PATH}/城市当日迁徙指数/{direction}/{city}.csv', encoding='utf-8_sig')
            city_internal_flow.to_csv(
                rf'{FILE_ROOT_PATH}/城内出行强度指数//{city}_{time.strftime("%Y%m%d", time.localtime())}.csv', encoding='utf-8_sig')

            city_work_index.to_csv(
                rf'{FILE_ROOT_PATH}/城市上班出行强度指数//{city}_{time.strftime("%Y%m%d", time.localtime())}.csv', encoding='utf-8_sig')

            city_entertainment_index.to_csv(
                rf'{FILE_ROOT_PATH}/城市就餐休闲出行强度指数//{city}_{time.strftime("%Y%m%d", time.localtime())}.csv', encoding='utf-8_sig')
        except:
